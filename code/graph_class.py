...
class Graph():
    ...
    def distance_matrix(self,method='shortest_path',changeInf=True,maxvaluemulti=10,force_recompute=True): 
        ...
        if (self.C is None) or force_recompute:
            ...
            elif method =='commute_distance':
                C = gwu.compute_commute_distance(A.toarray())
            elif method =='frozen_distance':
                C=gwu.compute_frozen_distance(A.toarray())
            elif method =='sif_distance':
                C=gwu.compute_sif_distance(A.toarray())
            elif method =='diffusive_distance':
                C=gwu.compute_diffusive_distance(A.toarray())
